...
def insightface_loss(embedding, labels, out_num, w_init=None, s=64., m=0.5,scope='insightface_loss', reuse=False):
    '''
...
def cosineface_loss(embedding, labels, out_num, w_init=None, s=30., m=0.4,scope='cosineface_loss', reuse=False):
    '''
    :param embedding: the input embedding vectors
    :param labels:  the input labels, the shape should be eg: (batch_size, 1)
...
def combine_loss(embedding, labels, batch_size, out_num, margin_a=1., margin_m=0.3, margin_b=0.2, s=64.):
    '''
...
def cos_loss(x, y, num_cls, reuse=False, alpha=0.35, scale=64, name='cos_loss'):
    ...
    xs = x.get_shape()
    with tf.variable_scope(name, reuse=reuse) as center_scope:
